...
% linear_movieLP=reshape(movieLP,size(movieLP,1).*size(movieLP,2),size(movieLP,3));
% diff_linear_movieLP=diff(linear_movieLP')';
% new_diff_linear_movieLP=zeros(size(movieLP,1).*size(movieLP,2),size(movieLP,3));
% new_diff_linear_movieLP(:,2:end)=diff_linear_movieLP;
% range=1:num_per_stim:num_trials.*num_per_stim;
% new_diff_linear_movieLP(:,range)=diff_linear_movieLP(:,range+1);
% 
% dff_movieLP=reshape(new_diff_linear_movieLP,size(movieLP,1),size(movieLP,2),size(movieLP,3));

% Baseline subtracted DFF
...
% figure
% imagesc(mean(prestim_F,3))
% colorbar
% %savefig([update_name 'Prestim_F']);
% 
% figure
% imagesc(mean(prestim_dFF,3))
% colorbar
% %savefig([update_name 'Prestim_dFF']);

%% If want to take first temporal derivative (to deconvolve)
% 
% linear_dFF=reshape(prestim_dFF,size(prestim_dFF,1).*size(prestim_dFF,2),size(prestim_dFF,3));
% diff_linear_dFF=diff(linear_dFF')';
% new_diff_linear_dFF=zeros(size(prestim_dFF,1).*size(prestim_dFF,2),size(prestim_dFF,3));
% new_diff_linear_dFF(:,2:end)=diff_linear_dFF;
% range=1:num_per_stim:num_trials.*num_per_stim;
% new_diff_linear_dFF(:,range)=diff_linear_dFF(:,range+1);
% 
% prestim_dFF=reshape(new_diff_linear_dFF,size(prestim_dFF,1),size(prestim_dFF,2),size(prestim_dFF,3));

%% STEP 2 - Define trial types
...
% imagesc(All_avg_movie(:,:,i))
% colormap('jet')
% colorbar
% caxis([-0.05 0.05]) % Bigger range, easier to notice changes
% % caxis([-0.02 0.02])
...
%ABCD_trials=find(event_history(1,:)==1);
